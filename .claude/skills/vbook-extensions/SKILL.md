...
`novel`/`comic` only: `config.js` sets `let BASE_URL = DOMAIN;`, then re-checks a legacy `CONFIG_URL` global in a `try/catch` for back-compat with the old app (undefined on this engine — the `ReferenceError` is caught, `BASE_URL` stays `DOMAIN`; if the old app does inject `CONFIG_URL`, it wins). Scripts that need the site's base URL do `load('config.js');` and use `BASE_URL`, never `DOMAIN` directly. Safe under constraint 2 because the name (`BASE_URL`) differs from the config key (`DOMAIN`) it aliases — no redeclaration collision. `video`/`tts`/`translate` templates use `DOMAIN` (or a hardcoded engine URL for `tts`/`translate`) directly, no alias, no back-compat shim.
